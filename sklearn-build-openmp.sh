...
CXX="gcc-5" CC="gcc-5" CFLAGS="-fopenmp -DCV_OMP=1" CXXFLAGS="-fofopenmp -DCV_OMP=1" LDFLAGS=-lgomp python setup.py build
if [ $? -ne 0 ]; then
	cd ..
    exit 1
fi
...
CXX="gcc-5" CC="gcc-5" CFLAGS="-fopenmp -DCV_OMP=1" CXXFLAGS="-fofopenmp -DCV_OMP=1" LDFLAGS=-lgomp python setup.py install
...
echo "**************************************************************************************************"
echo "Install scikit-learn"
FLAGS="-fopenmp -DCV_OMP=1" CXXFLAGS="-fofopenmp -DCV_OMP=1" LDFLAGS=-lgomp python setup.py install
